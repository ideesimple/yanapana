            ...
            @collection = @search.result.
              ...
              if params[:q][:s].include?("master_price")
                @collection = @collection.group("spree_variants.price")
 													...
 													if params[:q][:s].include?("sku")
                
															if params[:q][:s].include?("desc")
                @collection = @collection.group("variants_spree_products.sku", "spree_variants.id", "variants_spree_products.id", "spree_assets.id", "spree_option_values.id").order("spree_variants.sku DESC")
															else
																@collection = @collection.group("variants_spree_products.sku", "spree_variants.id", "variants_spree_products.id", "spree_assets.id", "spree_option_values.id").order("spree_variants.sku ASC")
															end
														end
          else
