  ...
  class Product < ActiveRecord::Base
    ...
    add_search_scope :descend_by_master_price do
      joins(:master).order("#{variant_table_name}.price DESC")
 			...
 			add_search_scope :ascend_by_sku do
      joins(:master).order("#{variant_table_name}.sku ASC")
    end

    add_search_scope :descend_by_sku do
      joins(:master).order("#{variant_table_name}.sku DESC")
    end
